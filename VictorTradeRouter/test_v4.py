...
class NavalTradeV4Tests(unittest.TestCase):
    ...
    def test_navigation_and_recommendation_labels_have_visible_backgrounds(self):
        app_source = Path(__file__).with_name("app.py").read_text(encoding="utf-8")
        self.assertIn(".stApp div.stButton > button *", app_source)
        self.assertIn('[data-testid="stButton"] > button *', app_source)
        self.assertIn("color:#fff4c9 !important", app_source)
        self.assertIn("-webkit-text-fill-color:#fff4c9 !important", app_source)
        self.assertIn("background:linear-gradient(#20516f,#103149) !important", app_source)
